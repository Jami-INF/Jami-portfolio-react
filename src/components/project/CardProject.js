...
const CardProject = ({ title, description, stack, imagePath, projectURL, direction }) => {
    ...
    return (
        <div className="p-4 mb-4 text-center">
            <Slide direction={isImageOnLeft ? "left" : "right"}>
                    ...
                    <div className="flex flex-col p-6">
                        ...
                        <div className="mb-8 justify-left flex flex-wrap">
                            {stack.split(",").map((tech, index) => (
                                <span key={index} className="border-2 border-slate-700 text-black dark:text-white font-bold py-2 px-4 rounded-full text-sm mr-2 mb-2">
                                    {tech}
                                </span>
                            ))}
                        </div>
